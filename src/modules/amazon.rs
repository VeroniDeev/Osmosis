use reqwest::{
    header::{HeaderMap, HeaderValue, COOKIE},
    Client,
};
use std::collections::HashMap;

pub struct Amazon {
    url: String,
    headers: HeaderMap,
    client: Client,
    account: HashMap<String, bool>,
}

impl Amazon {
    pub fn new() -> Self {
        Amazon {
            url: String::from("https://www.amazon.in/ap/signin"),
            headers: HeaderMap::new(),
            client: Client::new(),
            account: HashMap::new(),
        }
    }
    pub async fn start(&mut self, phonenumber: &str) -> Result<(), reqwest::Error> {
        let mut form_data: HashMap<&str, &str> = HashMap::new();

        form_data.insert("appActionToken", "Aok8C9I71Cr17vp22ONGvDUXR8Yj3D");
        form_data.insert("appAction", "SIGNIN_PWD_COLLECT");
        form_data.insert("subPageType", "SignInClaimCollect");
        form_data.insert(
            "openid.return_to",
            "ape:aHR0cHM6Ly93d3cuYW1hem9uLmluLz9yZWZfPW5hdl95YV9zaWduaW4=",
        );
        form_data.insert("prevRID", "ape:MzZDUDc2Q0dRNTJOM1RCMEhaRzg=");
        form_data.insert("workflowState", "eyJ6aXAiOiJERUYiLCJlbmMiOiJBMjU2R0NNIiwiYWxnIjoiQTI1NktXIn0.tCHWdlv4kSSigZCZiGfSCYgnReddxq7c0cUpf0dxYqYzWU-ZHIL0mQ.eP-cXQNtVyBr4q_g.fNRQAD5f18IU0nmqT7IwklJZV-_b60As-_dvyVd4MMjDpiMoGFJ0edbmuL8GJKT_BEE7ClwIpUYOtUtejr7v8qCRy4iD6bg_eBRSnTmZiXzVsx4EuL241zhoriZ7FpXS2seG82sx85C2udl1sPRQyKnO1zIqulOCechL_LzBmIRDv9ngzfij-nYmjWrDpZvAXiKCclR9v0UYh_SqjjOIrStMC53AlWjH-hYdDkXWSeTyHchFi9Ij4ndOgJb9tKNucA4_j7Uy-R0wvB9zlwEfQNa3394guXjjz6IR3TVMjw41bySCYbHLf6j5oj-5xh6UZm2CsW7DE5gqbHmlq5Nv8zLvTRTO9HJvM9Wr36R1eDRN.wZAX4qr9VTROJR9qdWbHfw");
        form_data.insert("email", phonenumber);
        form_data.insert("password", "");
        form_data.insert("create", "0");
        form_data.insert("metadata1", "ECdITeCs:");

        let cookies_string = "session-id=262-6899214-0753700; \
        session-id-time=2289745062l; \
        i18n-prefs=INR; \
        csm-hit=tb:6NWTTM14VJ00ZAVVBZ3X+b-36CP76CGQ52N3TB0HZG8|1659025064788&t:1659025064788&adb:adblk_no; \
        ubid-acbin=257-4810331-3732018; \
        session-token=\"tyoeHgowknphx0Y/CBaiVwnBiwbhUb1PRTvQZQ+07Tq9rmkRD6bErsUDwgq6gu+tA53K6WEAMwOb3pN4Ti3PSFoo+I/Jt5qIEDEMHIeRo1CrE264ogGDHsjge/CwWUZ9bVZtbo32ej/ZPQdm8bYeu6TQhca+UH7Wm9OOwBGoPl7dfoUk79QLYEz69Tt3ik4zMJom8jfgI227qMPuaMaAsw==\"";

        self.headers.insert(
            "User-Agent",
            HeaderValue::from_static(
                "Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:103.0) Gecko/20100101 Firefox/103.0",
            ),
        );
        self.headers.insert(
            "Accept",
            HeaderValue::from_static("text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8"),
        );
        self.headers.insert(
            "Accept-Language",
            HeaderValue::from_static("en-US,en;q=0.5"),
        );
        self.headers
            .insert("Accept-Encoding", HeaderValue::from_static("gzip, deflate"));
        self.headers.insert("DNT", HeaderValue::from_static("1"));
        self.headers.insert(
            "Content-Type",
            HeaderValue::from_static("application/x-www-form-urlencoded"),
        );
        self.headers
            .insert("Origin", HeaderValue::from_static("https://www.amazon.in"));
        self.headers
            .insert("Connection", HeaderValue::from_static("close"));
        self.headers
            .insert("X-Forwarded-For", HeaderValue::from_static("127.0.0.1"));
        self.headers.insert("Referer", HeaderValue::from_static("https://www.amazon.in/ap/signin?openid.pape.max_auth_age=0&openid.return_to=https%3A%2F%2Fwww.amazon.in%2F%3Fref_%3Dnav_ya_signin&openid.identity=http%3A%2F%2Fspecs.openid.net%2Fauth%2F2.0%2Fidentifier_select&openid.assoc_handle=inflex&openid.mode=checkid_setup&openid.claimed_id=http%3A%2F%2Fspecs.openid.net%2Fauth%2F2.0%2Fidentifier_select&openid.ns=http%3A%2F%2Fspecs.openid.net%2Fauth%2F2.0&"));
        self.headers
            .insert("Upgrade-Insecure-Requests", HeaderValue::from_static("1"));
        self.headers
            .insert("Sec-Fetch-Dest", HeaderValue::from_static("document"));
        self.headers
            .insert("Sec-Fetch-Mode", HeaderValue::from_static("navigate"));
        self.headers
            .insert("Sec-Fetch-Site", HeaderValue::from_static("same-origin"));
        self.headers
            .insert("Sec-Fetch-User", HeaderValue::from_static("?1"));

        let res = self
            .client
            .post(self.url.as_str())
            .headers(self.headers.clone())
            .header(COOKIE, cookies_string)
            .form(&form_data)
            .send()
            .await?;

        let stii = res.text().await?;

        if let Some(_) = stii.find("ap_change_login_claim") {
            self.account.insert(String::from("found"), true);
        } else {
            self.account.insert(String::from("found"), false);
        }

        if let Some(_) = stii.find("<html") {
            self.account.insert(String::from("rate_limit"), false);
        } else {
            self.account.insert(String::from("rate_limit"), true);
        }

        println!("{:?}", self.account);
        Ok(())
    }
}
